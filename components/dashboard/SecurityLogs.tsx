                ...
                <CardContent>
                    {logs.length === 0 ? (
                        <div className="text-center py-12 text-slate-500 font-mono text-sm">
                            Nenhum incidente de segurança registrado. Seus sistemas estão quietos... por enquanto.
                        </div>
                    ) : (
                            ...
                            <Table>
                                <TableHeader className="bg-slate-900">
                                    <TableRow className="border-slate-800 hover:bg-slate-900">
                                        <TableHead className="text-slate-400 font-mono">DATA/HORA</TableHead>
                                        <TableHead className="text-slate-400 font-mono">STATUS</TableHead>
                                        <TableHead className="text-slate-400 font-mono">ATACANTE</TableHead>
                                        <TableHead className="text-slate-400 font-mono">PROMPT INJETADO</TableHead>
                                        <TableHead className="text-slate-400 font-mono text-right">AÇÃO</TableHead>
                                    </TableRow>
                                </TableHeader>
                                <TableBody>
                                    {logs.map((log: any) => (
                                                ...
                                                <div className="flex items-center gap-2">
                                                    {log.success ? (
                                                        <span className="flex items-center gap-1 text-red-500 font-bold bg-red-950/30 px-2 py-1 rounded border border-red-900/50">
                                                            <ShieldAlert className="w-3 h-3" /> VIOLAÇÃO
                                                        </span>
                                                    ) : (
                                                        <span className="flex items-center gap-1 text-green-500 font-bold bg-green-950/30 px-2 py-1 rounded border border-green-900/50">
                                                            <ShieldCheck className="w-3 h-3" /> BLOQUEADO
                                                        </span>
                                                    )}
                                                </div>
                                            </TableCell>
                ...
                <DialogContent className="bg-slate-950 border-slate-800 max-w-2xl">
                    <DialogHeader>
                        <DialogTitle className="flex items-center gap-2 text-cyan-400 font-mono text-xl">
                            ...
                            RELATÓRIO DE ANÁLISE FORENSE #{selectedLog?.id}
                        </DialogTitle>
                        <DialogDescription className="font-mono text-xs text-slate-500">
                            Acesso autorizado apenas. Analisando vetor de ataque no Cofre #{selectedLog?.safe?.id ?? 'N/A'}.
                        </DialogDescription>
                    </DialogHeader>
                    ...
                    {selectedLog && (
                        <div className="space-y-6 mt-4 font-mono text-sm">
                            ...
                            <div className="grid grid-cols-2 gap-4 bg-slate-900/50 p-4 rounded border border-slate-800">
                                <div>
                                    <span className="text-slate-500 text-xs block mb-1">ID DO ATACANTE</span>
                                    <span className="text-purple-400 font-bold">{selectedLog.attacker.username}</span>
                                </div>
                                <div>
                                    <span className="text-slate-500 text-xs block mb-1">RESULTADO</span>
                                    <span className={selectedLog.success ? "text-red-500 font-bold" : "text-green-500 font-bold"}>
                                        {selectedLog.success ? "SISTEMA COMPROMETIDO" : "ATAQUE NEUTRALIZADO"}
                                    </span>
                                </div>
                            </div>
